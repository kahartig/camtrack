...
class TrajectoryFile:
    ...
    data: pandas DataFrame
        ...
        columns: grid #, year, month, day, hour, minute, fhour, lat, lon,
            height (m), <any diagnostic variables...>
    '''
...
class DataForPlotting(object):
    ...
    def __init__(self, *args):
        ...
        # dict containing descriptions of each variable as given in the netcdf
        # file
...
def select_trajectory(trajectory_number, all_trajectories):
    ...
    traj = all_trajectories.data[
        np.where(all_trajectories.data[:, 0] == trajectory_number)]
...
def read_netcdf_files(trajectory, trajectory_number, all_trajectories):
    ...
    month = all_trajectories.list_of_trajectories[
        'month'][trajectory_number - 1]
    ...
    # open h1 file
    nc_f = '../CAM-output-for-Arctic-air-suppression/PI/pi_3h.cam.h1.00' + \
        "%02d" % (year) + '-01-01-10800.nc'
    ds1 = xr.open_dataset(nc_f)

    # open h2 file
    nc_f = '../CAM-output-for-Arctic-air-suppression/PI/pi_3h.cam.h2.00' + \
        "%02d" % (year) + '-01-01-10800.nc'
    ds2 = xr.open_dataset(nc_f)

    # open h3 file
    nc_f = '../CAM-output-for-Arctic-air-suppression/PI/pi_3h.cam.h3.00' + \
        "%02d" % (year) + '-01-01-10800.nc'
    ds3 = xr.open_dataset(nc_f)

    # open h4 file
    nc_f = '../CAM-output-for-Arctic-air-suppression/PI/pi_3h.cam.h4.00' + \
        "%02d" % (year) + '-01-01-10800.nc'
    ...
    if not linear_interpolation:
        ...
        # this contains the indices of the data (time, lat, lon) along the
        # trajectory
